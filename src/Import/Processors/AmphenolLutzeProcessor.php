...
class AmphenolLutzeProcessor extends BaseProcessor {

	public static function get_manufacturer_slug(): string {
		return 'amphenol-lutze';
	}
	...
	/**
	 * attr labels that are noise/internal and must never become specs.
	 */
	private function get_ignored_attr_labels(): array {
		return [
			'de fas', // internal "De fas 01 luetze int fs01 order..." order-code rows
			'n o art',
			'estado del producto',
			'foto del producto', // disclaimer "Las fotos de los productos no son a escala..."
			'fecha de expiracion', // stale certificate/datasheet expiry dates
			'descripcion', // duplicates the product description column
			'u e', // cryptic DIN-rail/dimension reference, not a datasheet spec
		];
	}
	...
	public function process_row( array $row ): array {
		...
		// Resolve category path from DB structure by lutze category_id (the
		// hierarchical slug path, e.g. "Cable-&-Connectivity|Cables-de-control").
		// Works on incremental imports; on --mode=replace categories are rebuilt
		// by import-lutze-structure.php.
		$l1 = isset( $row['category_l1'] ) ? trim( (string) $row['category_l1'] ) : '';
		$l2 = isset( $row['category_l2'] ) ? trim( (string) $row['category_l2'] ) : '';
		$l3 = isset( $row['category_l3'] ) ? trim( (string) $row['category_l3'] ) : '';
		...
		if ( '' !== $l1 ) {
			...
			foreach ( $candidates as $cid ) {
				$cat_info = $this->get_category_path_by_lutze_id( $cid );
		...
		// Image: real mediabridge URLs, no placeholders in this CSV. Served
		...
		// Documents: datasheet + extra mediabridge downloads + 3D CAD
		$data['pdf'] = $this->parse_documents( $row );

		// Specs: attrs JSON (label/value rows) in Spanish, minus noise labels.
