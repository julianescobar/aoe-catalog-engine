<?php
/**
 * CLI: Import Amphenol Lutze category structure + re-attribute products.
 *
 * Reads tools/lutze_categorias.csv (L1 -> L2 -> L3 hierarchy, with lutze
 * category ids as hierarchical slug paths, descriptions, urls and part counts)
 * and tools/lutze_productos.csv (products) and rebuilds the aoe_catalog_categories
 * rows for amphenollutze:
 *   - L1 (level 1), L2 (level 2), L3 (level 3) with metadata lutze_id + url
 *   - hierarchical slugs (parent-slug + own-slug) so repeated names like
 *     "Accesorios" do not collide between branches
 *   - uncategorized fallback
 *   - re-attributes existing products to the rebuilt categories by SKU, using
 *     the product category_l1/l2/l3 columns to build the lutze id (falling back
 *     to a higher level when a product L3 code has no matching category row,
 *     e.g. junk codes like A2012)
 *   - prunes empty categories (bottom-up)
 *   - regenerates pages via pack_catalog
 *
 * IMPORTANT ORDER (like RF/Anytek/LTW):
 *   1) php tools/full-import.php --manufacturer=amphenollutze --csv=tools/lutze_productos.csv --mode=replace
 *   2) php tools/import-lutze-structure.php
 *
 * Usage:
 *   php tools/import-lutze-structure.php
 */
...
// ---- Parse products CSV (semicolon) -> SKU => lutze path candidates ----
$prods_by_sku = [];
...
foreach ( [ '1', '2', '3' ] as $level ) {
	foreach ( $cats as $aid => $row ) {
		...
		if ( $level !== '1' && $parent_aid !== '' && isset( $db_by_lutze[ $parent_aid ] ) ) {
			$parent_db = $db_by_lutze[ $parent_aid ];
		}
		$base_slug = sanitize_title( $row['name'] );
		$slug      = $base_slug;
		if ( $level !== '1' && '' !== $parent_aid && isset( $slug_by_lutze[ $parent_aid ] ) && '' !== $slug_by_lutze[ $parent_aid ] ) {
			$slug = $slug_by_lutze[ $parent_aid ] . '-' . $base_slug;
		}
		if ( '' === $slug ) {
		...
		$db_by_lutze[ $aid ] = (int) $wpdb->insert_id;
		$slug_by_lutze[ $aid ] = $slug;
		$counts[ 'L' . $level ]++;
	}
}
...
// ---- Re-attribute products by SKU -> lutze path (fallback up levels) ----
$remapped = 0;
$to_uncat = 0;
foreach ( $products as $p ) {
	...
	if ( isset( $prods_by_sku[ $sku ] ) ) {
		...
		// Try full path, then progressively shorter paths
		while ( '' !== $path ) {
			if ( isset( $db_by_lutze[ $path ] ) ) {
				$new_id = $db_by_lutze[ $path ];
				break;
			}
			$pos = strrpos( $path, '|' );
