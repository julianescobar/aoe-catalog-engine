<?php
/**
 * CLI: Import Amphenol Industrial category structure + re-attribute products.
 *
 * Reads tools/amphenol_industrial_categorias.csv (L1 -> L2 hierarchy, with
 * ind category ids as "L1|L2" composite paths, descriptions as JSON arrays of
 * paragraphs, urls and images) and tools/amphenol_industrial_productos.csv
 * (products) and rebuilds the aoe_catalog_categories rows for amphenolindustrial:
 *   - L1 (level 1) and L2 (level 2) with metadata ind_id + url
 *   - hierarchical slugs (parent-slug + own-slug) so repeated L2 names like
 *     "PowerLok" under different EV families do not collide
 ...
 *   - images from the CSV (remote amphenol-industrial.de URLs)
 *   - uncategorized fallback
 *   - re-attributes existing products by SKU, building the ind id from the
 *     product category_l1/l2 columns (falling back to L1)
 *   - prunes empty categories (bottom-up)
 *   - regenerates pages via pack_catalog
 *
 * IMPORTANT ORDER (like RF/Lutze):
 *   1) php tools/full-import.php --manufacturer=amphenolindustrial --csv=tools/amphenol_industrial_productos.csv --mode=replace
 *   2) php tools/import-ind-structure.php
 *
 * Usage:
 *   php tools/import-ind-structure.php
 */
...
// ---- Parse products CSV (semicolon) -> SKU => ind path candidates ----
$prods_by_sku = [];
...
foreach ( [ '1', '2' ] as $level ) {
	foreach ( $cats as $aid => $row ) {
		...
		if ( $level !== '1' && $parent_aid !== '' && isset( $db_by_ind[ $parent_aid ] ) ) {
			$parent_db = $db_by_ind[ $parent_aid ];
		}
		$base_slug = sanitize_title( $row['name'] );
		$slug      = $base_slug;
		if ( $level !== '1' && '' !== $parent_aid && isset( $slug_by_ind[ $parent_aid ] ) && '' !== $slug_by_ind[ $parent_aid ] ) {
			$slug = $slug_by_ind[ $parent_aid ] . '-' . $base_slug;
		}
		if ( '' === $slug ) {
		...
		$db_by_ind[ $aid ] = (int) $wpdb->insert_id;
		$slug_by_ind[ $aid ] = $slug;
		$counts[ 'L' . $level ]++;
	}
}
...
// ---- Re-attribute products by SKU -> ind path (fallback up levels) ----
$remapped = 0;
$to_uncat = 0;
foreach ( $products as $p ) {
	...
	if ( isset( $prods_by_sku[ $sku ] ) ) {
		...
		while ( '' !== $path ) {
			if ( isset( $db_by_ind[ $path ] ) ) {
				$new_id = $db_by_ind[ $path ];
				break;
			}
			$pos = strrpos( $path, '|' );
