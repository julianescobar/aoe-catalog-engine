<?php
/**
 * CLI: Import Amphenol Audio category structure + re-attribute products.
 *
 * Reads tools/amphenol-audio-categorias.csv (2-level hierarchy: L1 -> L2,
 * with path-style category_id, parent_id, name, url, image_url, description,
 * features, options, pdf_extra) and tools/amphenol-audio-productos.csv
 * (products) and rebuilds the aoe_catalog_categories rows for amphenol-audio:
 *   - L1 and L2 (levels 1, 2) with hierarchical slugs
 *     (parent-slug + own-slug)
 *   - descriptions from the CSV
 *   - images stored from CSV (remote amphenolaudio.com URLs)
 *   - metadata: audio_id, url, features (list), options (list)
 *   - uncategorized fallback
 *   - re-attributes existing products by SKU, building the category path from
 *     the product category_l1/l2 columns (full path L1|L2)
 *   - prunes empty categories (bottom-up)
 *   - regenerates pages via pack_catalog
 *
 * The category features/options are stored as real-newline-separated strings
 * so the render lists them as <li> bullets.
 *
 * IMPORTANT ORDER (like RF/Industrial):
 *   1) php tools/full-import.php --manufacturer=amphenol-audio --csv=tools/amphenol-audio-productos.csv --mode=replace
 *   2) php tools/import-audio-structure.php
 *
 * Usage:
 *   php tools/import-audio-structure.php
 */
...
$manufacturer_slug = 'amphenol-audio';
$cats_file         = __DIR__ . '/amphenol-audio-categorias.csv';
$prods_file        = __DIR__ . '/amphenol-audio-productos.csv';
...
// Paths are full-name paths used to disambiguate and to match products.
$name_by_id = [];
...
// ---- Parse products CSV -> SKU => full path (L1|L2) ----
$prods_by_sku = [];
...
echo "Deleting existing Audio categories...\n";
...
$slug_by_id  = [];   // audio category id => slug
$counts = [ 'L1' => 0, 'L2' => 0 ];
foreach ( [ '1', '2' ] as $level ) {
	foreach ( $cats as $aid => $row ) {
		...
		// features/options come as JSON arrays -> newline-joined strings.
		$features = $row['features'] ?? '';
		$feat_lines = json_decode( $features, true );
		$feat_text  = is_array( $feat_lines ) ? implode( "\n", array_map( 'trim', $feat_lines ) ) : '';

		$options = $row['options'] ?? '';
		$opt_lines = json_decode( $options, true );
		$opt_text  = is_array( $opt_lines ) ? implode( "\n", array_map( 'trim', $opt_lines ) ) : '';
