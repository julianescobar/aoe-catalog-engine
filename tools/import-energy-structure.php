<?php
/**
 * CLI: Import Amphenol Energy category structure + re-attribute products.
 *
 * Reads tools/amphenol-energy-categorias.csv (2-level hierarchy: L1 -> L2,
 * Neptune Connectors has no L2) and tools/amphenol-energy-productos.csv and
 * rebuilds the aoe_catalog_categories rows for amphenol-energy:
 *   - L1 and L2 (levels 1, 2) with metadata energy_id + url
 *   - hierarchical slugs (parent-slug + own-slug)
 *   - descriptions from the CSV
 *   - images stored from CSV (remote amphenolenergy.com URLs) — stored only,
 *     NOT used in the render yet
 *   - uncategorized fallback
 *   - re-attributes existing products by SKU, building the category path from
 *     the product category_l1/l2 columns (full path L1|L2; products with empty
 *     category_l2, e.g. Neptune, fall back to L1)
 *   - prunes empty categories (bottom-up)
 *   - regenerates pages via pack_catalog
 *
 * IMPORTANT ORDER (like RF/Industrial/PCD):
 *   1) php tools/full-import.php --manufacturer=amphenol-energy --csv=tools/amphenol-energy-productos.csv --mode=replace
 *   2) php tools/import-energy-structure.php
 *
 * Usage:
 *   php tools/import-energy-structure.php
 */
...
$manufacturer_slug = 'amphenol-energy';
$cats_file         = __DIR__ . '/amphenol-energy-categorias.csv';
$prods_file        = __DIR__ . '/amphenol-energy-productos.csv';
...
echo "Deleting existing energy categories...\n";
...
$slug_by_id  = [];   // energy category id => slug
$counts = [ 'L1' => 0, 'L2' => 0 ];
...
echo "\n=== Resumen ===\n";
echo "Insertadas L1={$counts['L1']} L2={$counts['L2']} | fallback: uncategorized | despues prune: $final_total\n";
