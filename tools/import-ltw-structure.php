<?php
/**
 * CLI: Import Amphenol LTW category structure + re-attribute products.
 *
 * Reads tools/categoriassamphenolltw.csv (L1 -> L2 hierarchy, with ltw category
 * ids/urls, descriptions and images) and tools/productosamphenolltw.csv (products)
 * and rebuilds the aoe_catalog_categories rows for amphenolltw:
 *   - L1 (level 1) and L2 (level 2) with metadata ltw_id + url
 *   - "category-*" junk rows (generic "Amphenol Technology ALTW" sections) are skipped
 ...
 *
 * IMPORTANT ORDER (like Anytek):
 *   1) php tools/full-import.php --manufacturer=amphenolltw --csv=tools/productosamphenolltw.csv --mode=replace
 *   2) php tools/import-ltw-structure.php
 *
 * Usage:
 *   php tools/import-ltw-structure.php
 */
...
while ( ( $r = fgetcsv( $fh, 0, ';' ) ) !== false ) {
	...
	$cid = $row['category_id'] ?? '';
	// Skip generic junk rows (no products, generic descriptions, unused sections)
	if ( preg_match( '/^category-\d+$/', $cid ) ) {
		continue;
	}
	$cats[ $cid ] = $row;
}
fclose( $fh );
echo "Categorias CSV (tras filtrar junk category-*): " . count( $cats ) . "\n";
...
echo "Deleting existing LTW categories...\n";
$wpdb->delete( $table_c, [ 'manufacturer_id' => $mfr_id ], [ '%d' ] );

// ---- Insert categories level by level ----
$db_by_ltw = []; // ltw_id => db_id
$slug_by_ltw = []; // ltw_id => slug
$counts = [ 'L1' => 0, 'L2' => 0 ];
foreach ( [ '1', '2' ] as $level ) {
	foreach ( $cats as $aid => $row ) {
		...
		if ( $level !== '1' && $parent_aid !== '' && isset( $db_by_ltw[ $parent_aid ] ) ) {
			$parent_db = $db_by_ltw[ $parent_aid ];
			$parent_slug = $slug_by_ltw[ $parent_aid ] ?? '';
		}
		$base_slug = sanitize_title( $row['name'] );
		// Hierarchical slug (L1-slug + L2-slug) so repeated L2 names like
		// "Accessory", "Large Size", "Middle Size" get unique URLs per family.
		$slug = ( $level !== '1' && '' !== $parent_slug ) ? $parent_slug . '-' . $base_slug : $base_slug;
		$meta = [
			'ltw_id' => $aid,
			'url'    => $row['url'] ?? '',
		];
		...
		$db_by_ltw[ $aid ] = (int) $wpdb->insert_id;
		$slug_by_ltw[ $aid ] = $slug;
		$counts[ 'L' . $level ]++;
	}
}
...
foreach ( $products as $p ) {
	...
	if ( isset( $prods_by_sku[ $sku ] ) ) {
		...
		if ( isset( $db_by_ltw[ $aid ] ) ) {
			$new_id = $db_by_ltw[ $aid ];
		}
	}
	if ( null === $new_id ) {
		$new_id = $uncat_db;
